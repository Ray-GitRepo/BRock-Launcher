...
DEFAULT_SETTINGS = {
    ...
    "background": "pop.png", "random_background": False,
    "auto_quit": True, "theme": "default.css",
    "show_notifications": True,
}
DEFAULT_PROFILE = {
    "username": "Player", "avatar_color": "#3fcf8e",
    "bio": "", "xbox_gamertag": None, "xbox_xuid": None, "xbox_linked": False,
    "launches": 0,
}
AVATAR_COLORS = [
    ("#6366f1","Indigo"), ("#8b5cf6","Violet"), ("#38bdf8","Sky"), ("#3fcf8e","Emerald"),
...
class Launcher(Gtk.Window):
    ...
    def __init__(self):
        ...
        self._log_tag_table = None   # populated after TextView is created
        self._load_css()
        self._build_ui()
    ...
    _FALLBACK_ICONS = {
        ...
        "storage.svg":"📦","import.svg":"↓","settings.svg":"⚙",
    }
    ...
    def _rebuild_profile_btn_inner(self, btn=None):
        ...
        p = load_profile()
        if p.get("random_avatar", False):
            p["avatar_color"] = random.choice([c for c, _ in AVATAR_COLORS])
            save_profile(p)
        col = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        col.set_halign(Gtk.Align.CENTER); col.set_valign(Gtk.Align.CENTER)
        dot = Gtk.Box(); dot.set_size_request(30, 30); dot.set_halign(Gtk.Align.CENTER)
        color = p.get("avatar_color", "#3fcf8e")
        self._apply_inline_css(dot, f".nav-profile-dot {{ background-color:{color}; border-radius:15px; min-width:30px; min-height:30px; }}")
    ...
    def _rebuild_home(self):
        ...
        if not version_ids:
            ...
            edit_icon_path = os.path.join(ASSETS, "icons", "edit.svg")
    ...
    def _rebuild_storage(self, restore_tab=None):
        ...
        self.storage_page.pack_start(self._page_header("Storage"), False, False, 0)
        ...
        tab_map = {}
        tab_map["rp"]     = _tab("Resource Packs",  self._storage_packs_panel(RESOURCE_DIR, "resource"), "rp")
        tab_map["bp"]     = _tab("Behaviour Packs",  self._storage_packs_panel(BEHAVIOUR_DIR, "behavior"), "bp")
    ...
    def _storage_packs_panel(self, packs_dir, ptype):
    ...
    def _create_import_page(self):
        ...
        card_area.pack_start(_card("Import Background", "Add a custom background image (.png, .jpg)", "importB.svg", self._import_background), False, False, 0)
        card_area.pack_start(_card("Import Theme", "Add a custom CSS theme file", "importT.svg", self._import_theme), False, False, 0)
    ...
    def _rebuild_settings(self):
        ...
        prof_s = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
        prof_s.set_margin_start(26); prof_s.set_margin_end(26); prof_s.set_margin_bottom(26)
        prof_s.pack_start(self._section_lbl("DISPLAY NAME"), False, False, 0)
        un_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        un_row.get_style_context().add_class("settings-row")
        un_lbl = Gtk.Label(label="Username", xalign=0); un_lbl.set_hexpand(True)
        un_entry = Gtk.Entry(); un_entry.set_text(profile.get("username","Player")); un_entry.set_size_request(200,-1)
        def _save_un(w): p=load_profile(); p["username"]=w.get_text().strip() or "Player"; save_profile(p); self._rebuild_profile_btn_inner()
        un_entry.connect("activate", _save_un); un_entry.connect("focus-out-event", lambda w,_: _save_un(w))
        un_row.pack_start(un_lbl, True, True, 0); un_row.pack_start(un_entry, False, False, 0)
        prof_s.pack_start(un_row, False, False, 0)
        prof_s.pack_start(self._section_lbl("AVATAR COLOR"), False, False, 0)
        col_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        col_row.get_style_context().add_class("settings-row")
        cr2 = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        for hc, hl in AVATAR_COLORS:
            b2 = Gtk.Button(); b2.set_size_request(26, 26); b2.set_tooltip_text(hl); make_clickable(b2)
            self._apply_inline_css(b2, f"button {{ background-color:{hc}; border-radius:13px; padding:0; min-width:26px; min-height:26px; }}")
            def _sc(_, c=hc): p = load_profile(); p["avatar_color"] = c; p["random_avatar"] = False; save_profile(p); self._rebuild_profile_btn_inner()
            b2.connect("clicked", _sc); cr2.pack_start(b2, False, False, 0)
        col_row.pack_start(Gtk.Label(label="Color", xalign=0), True, True, 0)
        col_row.pack_start(cr2, False, False, 0)
        prof_s.pack_start(col_row, False, False, 0)
        rand_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)
        rand_row.get_style_context().add_class("settings-row")
        rand_cb = Gtk.CheckButton(label="Random avatar color on startup")
        rand_cb.get_style_context().add_class("settings-check")
        rand_cb.set_active(load_profile().get("random_avatar", False)); rand_cb.set_hexpand(True)
        def _on_rand_avatar(w):
            p = load_profile(); p["random_avatar"] = w.get_active()
            if w.get_active(): p["avatar_color"] = random.choice([c for c, _ in AVATAR_COLORS])
            save_profile(p); self._rebuild_profile_btn_inner()
        rand_cb.connect("toggled", _on_rand_avatar); rand_row.pack_start(rand_cb, True, True, 0)
        prof_s.pack_start(rand_row, False, False, 0)
        prof_s.pack_start(self._section_lbl("BIO"), False, False, 0)
        bio_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        bio_row.get_style_context().add_class("settings-row")
        bio_lbl = Gtk.Label(label="Bio", xalign=0); bio_lbl.set_hexpand(True)
        bio_e = Gtk.Entry(); bio_e.set_text(profile.get("bio","")); bio_e.set_placeholder_text("Short bio…"); bio_e.set_size_request(200,-1)
        def _save_bio(w): p=load_profile(); p["bio"]=w.get_text().strip(); save_profile(p)
        bio_e.connect("activate",_save_bio); bio_e.connect("focus-out-event", lambda w,_: _save_bio(w))
        bio_row.pack_start(bio_lbl, True, True, 0); bio_row.pack_start(bio_e, False, False, 0)
        prof_s.pack_start(bio_row, False, False, 0)
        ...
        about.set_margin_start(40); about.set_margin_end(40)
        about.set_margin_top(30); about.set_margin_bottom(30)
        about.set_halign(Gtk.Align.CENTER)
        logo_l = Gtk.Label(); logo_l.set_markup("<span size='xx-large'>⛏</span>")
        about.pack_start(logo_l, False, False, 0)
        title_l = Gtk.Label()
        title_l.set_markup(f"<b><big>Minecraft Bedrock Launcher</big></b>\nVersion {LAUNCHER_VERSION}")
        title_l.set_justify(Gtk.Justification.CENTER)
        about.pack_start(title_l, False, False, 0)
        about.pack_start(Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL), False, False, 8)
        for line in (
            "An unofficial launcher for Minecraft Bedrock Edition on Linux.",
            "Supports APK import, Xbox Live accounts, storage management,",
            "server hosting, and custom themes.",
            "", "── Credits ──",
            "mcpelauncher-client  ·  GTK3 / PyGObject  ·  Mojang / Microsoft",
            "", f"Config: {CONFIG_DIR}", f"Game: {GAME}",
        ):
            l = Gtk.Label(label=line, xalign=0.5)
            l.get_style_context().add_class("version-id" if line not in ("── Credits ──","") else "settings-section")
            l.set_justify(Gtk.Justification.CENTER)
            about.pack_start(l, False, False, 0)

        t1 = _tab("Generic",   _sw(gen),    "generic")
        t1.get_style_context().add_class("tab-btn-active")
        _tab("Customize", _sw(cust),   "customize")
        _tab("Profile",   _sw(prof_s), "profile_s")
    ...
    def _profile_overview_panel(self, profile, display_name):
        ...
        color = profile.get("avatar_color","#3fcf8e")
        ...
        if profile.get("xbox_linked"):
            ...
            xbox_lbl.set_markup(f"<span color='#3fcf8e'>🎮 Xbox Live</span>  ·  <span color='#484f58'>XUID: {profile.get('xbox_xuid','—')}</span>")
    ...
    def _show_notif_popup(self, notification):
        ...
        outer.set_margin_start(18); outer.set_margin_end(14)
        ic = Gtk.Label(); ic.set_markup(f"<span size='large'>{icon}</span>"); ic.set_valign(Gtk.Align.START)
        outer.pack_start(ic, False, False, 0)
        ...
        bl.set_line_wrap(True); bl.set_max_width_chars(55)
        ...
        self._apply_inline_css(fr, f".notif-frame {{ background-color:{notif_bg}; border-radius:8px; border:1px solid rgba(255,255,255,0.06); }}")
    ...
    def _edit_profile_dialog(self):
        ...
        selected_color=[profile.get("avatar_color","#3fcf8e")]
